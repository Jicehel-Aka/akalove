...
- **`ä ö Ä Ö` manquants.** Le README annonçait la couverture allemande complète (« ä ö ü ß + majuscules »),
  mais `tools/generate_extended_font.py` n'avait jamais les 4 lignes correspondantes (contrairement à
  ë/ï/ü, déjà présents, construits avec la même technique). Corrigé : `font8x8_extended.h` passe de 41 à
  45 glyphes. Si vous avez déjà une copie de ce composant dans un autre projet, relancez-y
  `python3 tools/generate_extended_font.py` après avoir reporté ces 4 lignes (juste après
  `GLYPHS[0x00DC] = diaeresis_upper('U')`) :
  ```python
  GLYPHS[0x00E4] = diaeresis_lower('a')   # ä
  GLYPHS[0x00F6] = diaeresis_lower('o')   # ö
  GLYPHS[0x00C4] = diaeresis_upper('A')   # Ä
  GLYPHS[0x00D6] = diaeresis_upper('O')   # Ö
  ```
- **`font8x8_basic.h` sans `static`.** `char font8x8_basic[128][8] = {...};` provoquait une erreur
  d'édition de liens (« symbole défini deux fois ») dès que `gb_text_render.h` était inclus depuis plus
  d'un fichier `.cpp` du même exécutable. Corrigé en `static const unsigned char font8x8_basic[128][8]`
  (le passage à `unsigned char` supprime aussi un avertissement de narrowing sous `-Werror` pour les
  octets ≥ 0x80 ; aucun changement de comportement, tous les appelants castaient déjà en `unsigned char*`).
